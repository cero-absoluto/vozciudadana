<template>
  <div class="screen active" id="s-archivo">
    <div class="arch-hdr">
      <div class="arch-title">📁 Archivo</div>
      <div class="arch-sub">Convocatorias cerradas — memoria ciudadana</div>
    </div>
    ...
    <div class="arch-filters">
      ...
      <select v-model="filtroOrden" class="arch-sel">
        ...
        <option value="adheridos">Más adheridos</option>
      </select>
    </div>
    ...
    <div class="arch-list" v-if="!loading">
      <div v-if="protestasFiltered.length === 0" class="arch-empty">
        ...
        <div style="font-size:13px;color:var(--text2)">No hay convocatorias cerradas con estos filtros</div>
      </div>
      <div v-for="p in protestasFiltered" :key="p.id" class="arch-item" @click="$router.push(`/informe/${p.id}`)">
        ...
        <div class="arch-item-meta">
          ...
          <span>👤 {{ fmt(p.count ?? 0) }} adheridos</span>
          <span v-if="p.cities_count > 0">🏙️ {{ p.cities_count }} ciudades</span>
        </div>
        <div class="arch-item-footer">
          <span style="font-size:10px;color:var(--text2)">Ver informe final →</span>
        </div>
      </div>
    </div>
    ...
    <div v-if="loading" class="arch-loading">
      ...
      <div style="font-size:12px;color:var(--text2);margin-top:10px">Cargando archivo...</div>
    </div>
  </div>
</template>
...
const SCOPE_BADGE = {
  national: { cls: 'badge-nat', icon: '🏛️', label: 'Nacional' },
  regional: { cls: 'badge-reg', icon: '📍', label: 'Regional' },
  global:   { cls: 'badge-glob', icon: '🌍', label: 'Global' },
};
const scopeBadge = p => SCOPE_BADGE[p.scope] ?? { cls: '', icon: '📢', label: p.scope };
